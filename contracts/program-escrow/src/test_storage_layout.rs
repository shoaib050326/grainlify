...
mod test {
    ...
    use crate::{DataKey, PauseFlags, ProgramEscrowContract, ProgramEscrowContractClient, STORAGE_SCHEMA_VERSION};
    ...
    fn test_all_required_instance_keys_readable() {
        ...
        env.as_contract(&client.address, || {
            ...
            let _maint: bool = env.storage().instance().get(&DataKey::MaintenanceMode).unwrap();
            let _ro: bool = env.storage().instance().get(&DataKey::ReadOnlyMode).unwrap();
        });
    }
}
